...
Fine-tune different TTS models on the **same dataset** ([Common Voice Georgian](https://huggingface.co/datasets/NMikka/Common-Voice-Geo-Cleaned)), evaluate them on the **same benchmark** ([FLEURS Georgian](https://huggingface.co/datasets/google/fleurs), 979 test samples) with the **same metrics**, and determine which architecture works best for Georgian.
...
- **Training:** [NMikka/Common-Voice-Geo-Cleaned](https://huggingface.co/datasets/NMikka/Common-Voice-Geo-Cleaned) — ~21,421k clips from Mozilla Common Voice Georgian (24kHz)
